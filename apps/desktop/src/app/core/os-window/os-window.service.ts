...
function isValidChrome(chrome?: Partial<WindowChromeMetrics>): boolean {
  ...
  const isGeometryValid =
    finiteNonNegative(chrome.titleBarHeight) &&
    finiteNonNegative(chrome.controlsInsetStart) &&
    finiteNonNegative(chrome.controlsInsetEnd);

  const isScaleValid =
    typeof chrome.scaleFactor === 'number' &&
    Number.isFinite(chrome.scaleFactor) &&
    chrome.scaleFactor > 0;

  return isModeValid && areControlsValid && isSideValid && isGeometryValid && isScaleValid;
}
...
function validateStartupSnapshot(value: unknown): StartupSnapshot {
  ...
  if (
    !isBackdropValid ||
    !isValidChrome(snapshot.chrome as Partial<WindowChromeMetrics> | undefined)
  ) {
    return FALLBACK_SNAPSHOT;
  }
...
export class OsWindowService {
  ...
  public async setWindowInteractiveRegions(
  ...
  private async initialize(): Promise<void> {
    try {
      if (this.ipc.supportsIPCEvents()) {
        this.unlistenAppearanceChange = await this.ipc.listenWindowAppearanceChanged(
          ({ payload }) => this.applySnapshot(payload),
        );
      }
